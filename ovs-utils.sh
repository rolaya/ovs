...
ovs_restart()
{
  ...
  # record was first created (see "ovs_port_qos_create_max_rate")).
  # This command must be executed as "root" user (for now).
...
ovs_port_qos_max_rate_create()
{
  ...
  if [[ $# -eq 2 ]] && [[ $2 -gt 1 ]]; then
    ...
    echo "UUIDs:             [$uuids] (first one is QoS record)"
    #echo "QoS record uuid:   [$qos_uuid]"
    #echo "Queue record uuid: [$queue_uuid]"
...
ovs_port_qos_max_rate_update()
{
  ...
  if [[ $# -eq 2 ]] && [[ $2 -gt 1 ]]; then
    ...
    # Format and execute traffic shaping command (creates and initializes new record in QoS table)
    command="sudo ovs-vsctl set QoS $uuid other_config:max-rate=$port_max_rate"
    echo "excuting: [$command]"
    uuids="$($command)"

    echo "Update QoS configuration:"
    echo "UUID:              [$uuid]"
    echo "ether max rate:    [$port_max_rate]"

  else
    echo "Usage: ovs_port_qos_max_rate_update uuid max-rate (e.g. sudo ovs-vsctl set QoS bdc3fe06-edcc-419b-80bd-d523a0628aa2 other_config:max-rate=30000000)..."
  fi
}
